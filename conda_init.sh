#!/bin/bash
eval "$(/work/Bachelor_my_workspace/miniconda3/bin/conda shell.bash hook)"
conda init
export MY_ENV=bachelor_environment
echo "conda activate $MY_ENV" >> ~/.bashrc


BASE_DIR="/work/Bachelor_my_workspace"
...
git lfs install
cd /work/Bachelor_my_workspace/Bachelor_project
git lfs pull