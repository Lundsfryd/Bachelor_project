...
```{r}
model_blame_time_0 <- glmmTMB(total_blame ~ 1  + (1|party) + offset(log(total_nr_sentences)), 
                           family = nbinom2, 
                           data = monthly_blame)
```
...
```{r}
anova(model_blame_time_0, model_blame_time_1, model_blame_time_2, model_blame_time_3)
```
...
```{r}
model_blame_time_18 <- glmmTMB(total_blame ~ scale(month_index)  + (1|party) + (1|in_gov) + offset(log(total_nr_sentences)), 
                           ziformula = ~1,
                           family = nbinom1, 
                           data = monthly_blame_18)
...
```{r}
model_blame_time_18_2 <- glmmTMB(total_blame ~ scale(month_index) + I(scale(month_index)^2)  + (1|party) + (1|in_gov) + offset(log(total_nr_sentences)), 
                           ziformula = ~1,
                           family = nbinom2, 
                           data = monthly_blame_18)
...
```
```{r}
anova(model_blame_time_18, model_blame_time_18_2)
```
...
```{r}
anova(election_model, election_model_quad, election_model_triq)
```
